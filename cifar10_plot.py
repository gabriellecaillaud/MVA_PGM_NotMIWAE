...
def plot_images_with_imputation(model_path, calib_config):
    ...
    for i in range(4):
        ...
        # Mask showing unchanged and modified pixels
        axes[i, 3].imshow(img_mask_batch[i].permute(1, 2, 0).numpy())
        axes[i, 3].set_title("After imputation")
        axes[i, 3].axis("off")

    plt.tight_layout()
    plt.savefig(f"temp/plot_{model_path}_res_{date}.png")

if __name__=="__main__":
    model_path = ""
    calib_config = {'n_hidden' : 512, 'n_latent': 128, 'missing_process': 'selfmasking', 'out_dist': 'gauss'}

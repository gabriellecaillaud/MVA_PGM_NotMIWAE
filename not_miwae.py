...
class BernoulliDecoderMiss(nn.Module):
    def __init__(self, n_latent, n_hidden, d, missing_process):
        ...
        if missing_process == 'nonlinear':
            self.fc1 = nn.Linear(in_features=d, out_features=n_hidden)  # Linear case: 1 dense layer
            self.fc2 = nn.Linear(in_features=n_hidden, out_features=d)  # TODO change output dim # Nonlinear: hidden layer
